...
sas.utils = sas.utils || {}, sas.events = sas.events || {}, sas.cmp = sas.cmp || {}, Date.now || (Date.now = function () {
    ...
    for (var e = 1; e < arguments.length; e++) {
        var a = arguments[e];
        if (a && "object" == typeof a) for (var s in a) void 0 !== a[s] && (Array.isArray(a[s]) ? t[s] = a[s] : "object" == typeof a[s] ? t[s] = sas.utils.extend({}, t[s], a[s]) : t[s] = a[s])
    }
    return t
    ...
    var i = function () {
        if (!n) {
            for (var t in a) if (!a[t].status) return;
            n = !0;
    ...
    for (var e = {}, a = t.split("; "), s = 0; s < a.length; s++) {
        var n = a[s], r = n.indexOf("="), i = n.substring(0, r), o = n.substring(r + 1);
        e[i] = o
    }
    return e
    ...
    sas.events._events[t] = sas.events._events[t] || {que: []};
    var s = sas.events._events[t];
    a && (s = s[a] = s[a] || {que: []}), s.que.push(e)
}, sas.events.fire = function (t, e, a) {
    sas.events._history.push({eventName: t, data: sas.utils.extend({timestamp: Date.now()}, e), id: a});
    var s = sas.events._events[t];
    if (s) {
        if (a && s[a]) for (var n = 0; n < s[a].que.length; n++) s[a].que[n](sas.utils.extend({}, e), a);
    ...
    if (!sas.__cmpBootstrapLoaded) {
        sas.__cmpBootstrapLoaded = !0, sas.events.on("CmpCalled", function () {
            a && clearInterval(a), setTimeout(function () {
                0 == sas.events.history("CmpAnswered").length && 0 == sas.events.history("CmpFailToAnswer").length && sas.events.fire("CmpFailToAnswer", {failType: "Too long"})
            }, 1500)
        });
        var t = {};
        sas.bootstrapGdpr = function () {
            void 0 !== window.__cmp && "[object Function]" === Object.prototype.toString.call(window.__cmp) ? (sas.events.fire("CmpCalled", {location: "Same Iframe"}), window.__cmp("getConsentData", null, n)) : (sas.events.fire("CmpNotFoundInFrame"), function () {
                ...
                e && (sas.events.fire("CmpFrameFound"), sas.events.addEvent(window, "message", s), sas.events.fire("CmpCalled", {location: "Different iframe"}), a = "getConsentData", r = null, i = function (t, e) {
                    n(t, e)
        ...
        }, sas.events.fire("CmpSearch", {firstTry: !0}), sas.bootstrapGdpr();
        var e = 0, a = setInterval(function () {
            "function" == typeof sas.bootstrapGdpr && 0 == sas.events.history("CmpCalled").length && (sas.events.fire("CmpSearch", {retryNumber: e}), sas.bootstrapGdpr()), (e > 50 || sas.events.history("CmpCalled").length > 0) && clearInterval(a), e++
        }, 500)
    }
    ...
    if (!sas.__smartLoaded) {
        ...
        var a = function (t) {
            var e = this;
            e.instances = [], sas.events.on("load", function () {
        ...
        a.prototype.onCall = function (t, e, a) {
            ...
            if (this.displayData = null, t ? this.renderStatus = null : "rendered" == this.renderStatus && (this.renderStatus = "pending"), sas.events.fire("reset", {
                ...
                tagId: this.ad.tagId,
                hardReset: t
            }, this.ad.tagId), e) for (var a = 0; a < this.instances.length; a++) this.instances[a].reset(t)
            ...
            if (a) {
                var s = a.childNodes.length > 1;
                t.sas_loadHandler && t.sas_loadHandler({
                    id: this.ad.formatId,
                    hasAd: s
            ...
            if ("rendered" != this.renderStatus) if (this.displayData) {
                ...
                var a = {formatId: this.ad.formatId, instance: this.ad.instance, tagId: this.ad.tagId};
                ...
                if (!this.displayData.scriptSrc && !this.displayData.content) return sas.events.fire("load", a, this.ad.tagId), void sas.events.fire("noad", a, this.ad.tagId);
        ...
        var s = function () {
            ...
            }, i = "http://www.smartadserver.com", o = r(), d = !0, c = encodeURIComponent, l = decodeURIComponent, p = 0,
            f = !1, u = 0, h = !1, g = null, v = {}, m = [], y = {}, I = {}, _ = {}, w = null;
        sas._networks = sas._networks || {}, sas._pendingCommands = sas._pendingCommands || {}, sas.callType = {
            ...
            XML: "xml",
            PASSBACK: "passback"
        }, sas.renderMode = {DEFAULT: 0, READY: 1, ON_DEMAND: 2};
        var T = !1, E = [], C = {onLoad: s, onError: s, onClean: s, beforeRender: s};
        ...
        }, A = "invalid site id", L = "invalid page id or name", N = "invalid format id", x = function (t) {
            throw new Error(t)
        };
        ...
        var b = function (t) {
            ...
            if (e) return e;
            var s = t.split("_"), n = s.slice(0, s.length - 1).join("_");
            if (s.length > 1 && !isNaN(s[s.length - 1]) && (e = v[n] || v["sas_" + n])) {
                var r = parseInt(s[s.length - 1]), i = e.ad.tagId + "_" + r, o = new a(i);
            ...
            i.id = "sas_script" + M++, i.type = "text/javascript", i.src = "function" == typeof a ? a() : a, i.async = s, s ? (sas.events.addLoadEvent(i, n), sas.events.addEvent(i, "error", r)) : (i.setAttribute("onload", B(n) + "()"), i.setAttribute("onError", B(r) + "()")), s ? t.appendChild(i) : e.write(i.outerHTML)
        }, H = function (e, a) {
            a = sas.utils.extend({forceMasterFlag: !1}, a), d = !!a.forceMasterFlag || d, o = a.resetTimestamp ? r() : o, W();
            var s = J(), n = nt() || t.location.origin + t.location.pathname, i = screen.height, c = screen.width;
            return function (t, e, a, s, n, r, i) {
                ...
                for (var c = 0; c < t.formats.length; c++) {
                    var l = t.formats[c], p = l.tagId ? l.tagId : "sas_" + l.id, f = _[p] || {};
            ...
            s = sas.utils.extend({forceMasterFlag: !1}, s), d = !!s.forceMasterFlag || d, o = s.resetTimestamp ? r() : o, e == sas.callType.ONECALL && W();
            var n = J(), i = nt() || t.location.origin + t.location.pathname, c = screen.height, l = screen.width;
            return function () {
        ...
        sas.events.addMessageEvent(t, function () {
            if (arguments && !(arguments.length < 1)) {
                ...
                if (e && e.data && "string" == typeof e.data && !(e.data.indexOf("SMRT") < 0)) {
                    var a = e.data.split(" ");
                    if (!(a.length < 2)) {
                        var s = a[2] || "", n = a[1].split(".");
                        if (0 !== n.length) {
                            for (var r = t, i = 0; i < n.length; i++) if (!(r = r[n[i]])) return;
                            try {
                                r(s)
                            } catch (t) {
                            }
        ...
        }, K = !1, $ = new sas.utils.Latch(["loaded"]);
        sas.setup = function (e) {
            if (K) throw new Error("sas.setup can only be called once");
            K = !0, i = (e = e || {}).domain || "http://www.smartadserver.com", f = e.async || f, sas_ajax = f, p = e.networkid || p, u = e.renderMode || u, h = e.inSequence || h, g = e.uid, I = sas.utils.extend(I, e.partnerIds), t.sas_renderMode = u, sas.configure({id: p}), f && u != sas.renderMode.DEFAULT || $.notify("loaded"), u == sas.renderMode.ON_DEMAND && (X = setTimeout(function () {
                sas.render()
            }, parseInt(e.renderModeTimeout) || 5e3))
        }, sas.call = function (t, e, s) {
            "string" != typeof t && (s = sas.utils.extend({}, e, {async: !0}), e = t, t = "std", u == sas.renderMode.DEFAULT && $.notify("loaded")), s = sas.utils.extend({}, s), t == sas.callType.ONECALL && e.formats && (s.async = !0), (e = sas.utils.extend({}, e)).siteId = e.siteId || e.siteid, e.pageId = e.pageId || e.pageid, e.pageName = e.pageName || e.pagename, e.formatId = e.formatId || e.formatid, e.siteId || x(A);
            var n, r,
                o = (n = navigator.userAgent.indexOf("iPad") > 0, r = "" + navigator.userAgent.indexOf("iPhone") > 0, n || r ? n ? "ipad" : "iphone" : navigator.userAgent.indexOf("Android") > 0 ? "android" : "");
            ...
            if (!Y(e, s)) {
                ...
                if (t == sas.callType.ONECALL) {
                    if (e.isOnecallJSON = !!e.formats, !e.formats) {
                        e.formats = [];
                        for (var c = ("" + e.formatId).replace(/\s/g, "").split(","), l = 0; l < c.length; l++) e.formats.push({id: c[l]})
                    }
                ...
                s.async && u == sas.renderMode.READY && (V || (V = !0, G.push(function () {
                    ...
                    var a, n;
                    a = {callType: t, ad: e, options: s}, n = function () {
                        if (E.length > 0) {
                            T = !0;
                            var t = E.shift(), e = t.options.onLoad;
                            t.options.onLoad = function (t) {
                                n(), e(t)
                            }, sas.events.fire("call", {callType: t.callType, ad: t.ad, options: t.options}, t.callType)
                        } else T = !1
                    }, E.push(a), T || n()
                }) : sas.events.fire("call", {callType: t, ad: e, options: s}, t)
            }
        }, sas.passback = function (t) {
            y[t.formatId] = {current: -1, ad: t};
            var e = b(t.formatId);
            if (e.options.onNoad) {
                var a = e.options.onNoad;
                e.options.onNoad = function (e) {
                    (new Image).src = t.noadUrl, a({formatId: e.formatId, tagId: e.tagId})
                }
            } else e.options.onNoad = function () {
                (new Image).src = t.noadUrl
            };
            sas.next(t.formatId)
        }, sas.next = function (t) {
            var a = b(t), s = y[a.ad.formatId];
            if (a && s) {
        ...
        sas.refresh = function (t, e) {
            ...
            if (0 == arguments.length) for (var e in v) {
                var a;
                (a = v[e]).clean()
            } else (a = b(t)) && a.clean()
        }, sas.noad = function (t) {
            ...
            if (0 == arguments.length) {
                if (clearTimeout(X), !$.isComplete() && (u == sas.renderMode.READY && n || u == sas.renderMode.ON_DEMAND)) for (var e in $.notify("loaded"), v) {
                    var a;
            ...
            if (Range && Range.prototype.createContextualFragment) s.appendChild(e.createRange().createContextualFragment(a)); else {
                var n = e.createElement("div");
                n.innerHTML = a;
                for (var r = n.childNodes, i = 0; i < r.length; i++) s.appendChild(st(r[i]))
            }
        }, sas.configure = function (t) {
            if (t && t.id && !(parseInt(t.id) <= 0) && !sas._networks[t.id] && (sas._networks[t.id] = t, sas._pendingCommands[t.id])) for (var e; e = sas._pendingCommands[t.id].shift();) e()
        ...
        var Z = function (t) {
            ...
            return {scriptType: s.tagName.toLowerCase(), scriptSrc: s.src, width: s.width || 0, height: s.height || 0}
        };
        ...
        }), t.sas_cleanAds = sas.clean, t.sas_cleanAd = sas.clean, sas.events.fire("Bootstrap", {time: "Start"}), sas.cmd = sas.cmd || [], sas.cmd.push = function (t) {
            t()
        };
        for (var et = 0; et < sas.cmd.length; et++) sas.cmd[et]();
        sas.events.fire("Bootstrap", {time: "End"})
    }
    ...
    function st(t) {
        var a;
        if ("script" == t.tagName.toLowerCase()) a = e.createElement("script"), t.type && (a.type = t.type), t.src && (a.src = t.src), t.text && (a.text = t.text); else {
            a = t.cloneNode(!1);
    ...
    function nt() {
        ...
        try {
            for (; e.parent.document;) {
                if (e.parent.document === e.document) return e.location.origin + e.location.pathname;
                e = e.parent
            }
        } catch (t) {
        }
        try {
            try {
                if (t.top.location.href) return t.top.location.origin + t.top.location.pathname
            } catch (t) {
            }
    ...
    function rt(t) {
        var a, s = e.createElement("a"), n = /(?:^\?|&)([^=&]+)=?([^&]*)(?=&|$)/g;
        s.href = t;
        for (var r = s.search, i = []; null !== (a = n.exec(r));) {
            "extuid-" === (p = {n: l(a[1]), v: l(a[2])}).n.substr(0, 7) && I[p.n.substr(7)] || i.push(p)
        }
        for (var o in I) i.push({n: "extuid-" + o, v: I[o]});
        r = "";
